...
namespace HexaSort.Services
{
    public class FirebaseAuthService : MonoBehaviour
    {
        ...
        {
            var validation = ValidateInput(email, password);
            ...
            var result = await SendRequest(SIGN_UP_URL + config.apiKey, request);
        ...
        public async Task<AuthResult> SignInAsync(string email, string password)
        {
            ...
            var result = await SendRequest(SIGN_IN_URL + config.apiKey, request);
        ...
        public async Task<AuthResult> ResetPasswordAsync(string email)
        {
            ...
            try
            {
                using var webRequest = CreatePostRequest(PASSWORD_RESET_URL + config.apiKey, json);
        ...
        private string ParseError(string responseBody)
        {
            try
            {
                var error = JsonUtility.FromJson<FirebaseErrorResponse>(responseBody);
