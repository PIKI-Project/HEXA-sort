...
#include "TMPro_Properties.cginc"
            ...
            pixel_t VertShader(vertex_t input) {
                ...
                const float outline =
                        _OutlineWidth * _ScaleRatioA * 0.5 * scale;
                ...
                fixed4 faceColor =
                        fixed4(input.color.rgb, opacity) * _FaceColor;
...
#if (UNDERLAY_ON | UNDERLAY_INNER)
                layerScale /= 1 + (
                    (_UnderlaySoftness * _ScaleRatioC) * layerScale);
                float layerBias =
                        (.5 - weight) * layerScale - .5 - (
...
#if (UNDERLAY_ON || UNDERLAY_INNER)
                output.texcoord1 = float4(input.texcoord0 + layerOffset,
          input.color.a, 0);
...
#if UNDERLAY_ON
                d = tex2D(_MainTex, input.texcoord1.xy).a * input.underlayParam.
                         x;
                c += float4(_UnderlayColor.rgb * _UnderlayColor.a,
               _UnderlayColor.a) * saturate(
        ...
        x;
                c += float4(_UnderlayColor.rgb * _UnderlayColor.a,
                    _UnderlayColor.a) * (1 - saturate(
                    d - input.underlayParam
                    .y)) * sd * (1 - c.a);
#endif
